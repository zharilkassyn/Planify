...
export async function generatePresentationStructure({
  ...
  const prompt = [
    ...
    'ОЧЕНЬ ВАЖНО ДЛЯ ДИЗАЙНА: пиши как для красивых слайдов, а не как для реферата.',
    'title: максимум 42 символа. subtitle: максимум 72 символа.',
    'content: 3 коротких тезиса, каждый максимум 6 слов или 46 символов. Никаких длинных абзацев.',
    'stats.label, timeline.text, comparison.left/right: тоже короткие фразы до 46 символов.',
    'quote: максимум 95 символов.',
    'Если мысль длинная, раздели её на несколько коротких слайдов, но не растягивай текст внутри одного блока.',
    ...
    body: {
      ...
      system: buildPlanifySystemPrompt({
        ...
        appState: [
          ...
          `Уровень информации: ${INFO_LABELS[informationLevel]}`,
          `Аудитория: ${AUDIENCE_LABELS[audience]}`,
          `Выбранный шаблон используется только как внутренний дизайн, его название и цвета нельзя писать на слайдах: ${templateText}`,
        ].join('\n'),
      }),
    },
  });
